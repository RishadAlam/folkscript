...
class GenerateOgImage implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $logo = 'data:image/svg+xml;base64,'.base64_encode(file_get_contents(public_path('images/folkscript-web-primary.svg')));
        $html = '<!doctype html><html><head><meta charset="utf-8"><style>'.$fontCss.'*{box-sizing:border-box}body{margin:0;width:1200px;height:630px;background:#FFFFFF;color:#1E2A47;font-family:"Lexend Variable",system-ui,sans-serif;padding:62px 76px;display:flex;flex-direction:column}header{border-bottom:1px solid #1e2a4733;padding-bottom:30px}header img{display:block;width:244px;height:50px}.title-slot{flex:1;min-height:0;display:flex;align-items:center;margin:24px 0}h1{font-weight:500;font-size:58px;line-height:1.2;letter-spacing:-.6px;margin:0;max-width:100%;overflow-wrap:anywhere}footer{font-size:22px;display:flex;justify-content:space-between;gap:28px;align-items:center}.muted{font-size:16px;color:#626771}</style></head><body><header><img src="'.$logo.'" width="244" height="50" alt="Folkscript"></header><main class="title-slot"><h1>'.$title.'</h1></main><footer><span>'.$author.'</span><span class="muted">Written by the people, read by everyone.</span></footer></body></html>';
