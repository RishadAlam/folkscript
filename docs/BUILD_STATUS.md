...
This is a locally implemented publishing application, with demonstration editorial content. It is not a deployed production service. The long-range reference is `docs/folkscript-build-plan.md`; the table below distinguishes implemented application paths from integrations that need an operator-owned service.
...
| Authorization | Server-side publishing policies, verified writer access, role administration, suspension, moderation, premium-body gates, audited read-only super-admin support sessions and an administrator-only Horizon dashboard. | Choose initial production administrators and operational access policy. |
...
| Membership | Cashier checkout and customer portal handlers, Stripe webhook support and server-side paywall checks. Stripe Connect onboarding/balance lookup, approved earnings ledger, protected transfer/reconciliation actions and author earnings history. | Stripe account approval, recurring prices, webhook secret/event setup, test-mode validation and a decided creator revenue policy before real transfers. |
...
- Unpublished or suspended-author stories are excluded by server-side queries. Public API and feed responses do not serialize member-only bodies.
...
- Read-only support sessions expire after 30 minutes, recheck the original operator's authority on each request, and block account/billing pages and mutations, including signed email-verification GET requests.
- External integrations remain off or return actionable unavailable states when their configuration is missing. Placeholder membership prices do not constitute a live paid product.
...
- Creator earnings are explicitly approved allocations. Automatic engagement-based revenue sharing is not implemented; the operator must decide and reconcile the financial allocation policy before recording earnings. Transfers remain disabled until enabled in the deployment environment.
- Optional reader tipping is not implemented. Video/embed support is restricted to the supported providers rather than arbitrary third-party HTML.
...
- No production deployment, DNS change, mail delivery, live subscription, payout, search-console submission, or external analytics capture has been performed.
