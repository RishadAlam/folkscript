...
Administration uses a compact application hierarchy within the same family: page headings are 30px/1.2 at weight 550, reducing to 27px at 480px; panel headings are 20px/1.3. Navigation and ordinary action labels are 14px, fields remain 16px, and secondary account/status metadata is 12–13px. These are administration-specific sizes, not replacements for the publication's editorial headings.
...
`admin.css` uses shared Fraunces, ink, paper, rules, and status colors, with tonal panels and no new decorative shadows. Navigation and primary actions have 44px minimum targets; mobile navigation summaries have 48px targets. Administration buttons suppress the publication's hover movement. Focus uses a 2px ink outline with a 3px offset. Platform settings and payout rows retain unavailable-service explanations and explicit financial context.
