...
<div class="page-shell content-page notifications-page"><div class="page-heading"><div><h1>{{ __('Notifications') }}</h1><p>{{ __('New followers and replies to your stories.') }}</p></div>@if(auth()->user()->unreadNotifications()->exists())<form method="POST" action="/notifications/read">@csrf<button class="btn btn-outline btn-small"><x-icon name="check-check" size="16" /> {{ __('Mark all as read') }}</button></form>@endif</div>
...
@empty<div class="empty-state"><x-icon name="bell" size="32" /><h2>{{ __('No notifications yet.') }}</h2><p>{{ __('When someone follows you or responds to a story, it will appear here.') }}</p><a href="/explore" class="btn btn-outline">{{ __('Explore stories') }}</a></div>@endforelse<div class="pagination-wrap">{{ $notifications->links() }}</div></div>
</x-layout>
