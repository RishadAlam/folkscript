    ...
    <nav class="admin-filter-tabs" aria-label="{{ __('Filter comments by status') }}">@foreach($commentStatusLabels as $value => $label)<a href="{{ route('admin', ['view' => 'comments', 'comment_status' => $value]) }}" @if($commentStatus === $value) aria-current="page" @endif>{{ $label }} @if($commentStatus === $value)<span>{{ $moderationComments->total() }}</span>@endif</a>@endforeach</nav>
        @forelse($moderationComments as $comment)
            <article class="moderation-item admin-comment">
                ...
                <form method="post" action="{{ route('admin.comments.update', array_merge([$comment], $returnQuery)) }}" class="moderation-actions">@csrf @method('PATCH')@if($comment->status !== 'visible')<button class="btn btn-primary" name="status" value="visible">{{ $comment->status === 'hidden' ? __('Restore comment') : __('Approve comment') }}</button>@endif @if($comment->status !== 'hidden')<button class="btn btn-outline" name="status" value="hidden">{{ __('Hide comment') }}</button>@endif</form>
            </article>
        @empty
