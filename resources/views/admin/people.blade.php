    ...
    <p class="admin-result-count">{{ trans_choice('{0} No accounts|{1} 1 account|[2,*] :count accounts', $users->total()) }}</p>
            <div class="admin-table-wrap"><table class="admin-table admin-responsive-table admin-people-table"><caption class="sr-only">{{ __('Community accounts and access controls') }}</caption><thead><tr><th scope="col">{{ __('Person') }}</th><th scope="col">{{ __('Account') }}</th><th scope="col">{{ __('Access') }}</th></tr></thead><tbody>
                @forelse($users as $person)
                    ...
                    @endphp
                    <tr><td class="admin-person-cell"><div class="admin-person-identity"><x-avatar :user="$person" size="small" /><div><strong>{{ $person->name }} @if($person->id === auth()->id())<span class="admin-you">{{ __('You') }}</span>@endif</strong><span class="muted table-secondary">{{ $person->email }}</span><a class="admin-profile-link" href="{{ '/@'.$person->username }}">{{ __('View profile') }}</a></div></div></td><td class="admin-person-status"><span class="admin-role-name">{{ __('ui.roles.'.$primaryRole) }}</span><span @class(['admin-status', 'is-danger' => (bool) $person->suspended_at, 'is-success' => !$person->suspended_at && $person->hasVerifiedEmail()])>{{ $person->suspended_at ? __('Suspended') : ($person->hasVerifiedEmail() ? __('Verified') : __('Unverified')) }}</span></td><td class="admin-person-access">
                        @if($canEditAccess)
                            ...
                            <form method="post" action="{{ route('admin.users.update', array_merge([$person], $returnQuery)) }}" class="user-access-form" data-error-bag="{{ $accessBag }}">@csrf @method('PATCH')<input type="hidden" name="access_person" value="{{ $person->id }}">
                                <label class="field"><span>{{ __('Role') }}</span><select class="form-input" name="role" aria-label="{{ __('Role for :name', ['name' => $person->name]) }}" aria-invalid="{{ $accessErrors->has('role') ? 'true' : 'false' }}" aria-describedby="role-error-{{ $person->id }}">@foreach(['reader' => 'Reader', 'author' => 'Author', 'editor' => 'Editor'] + (auth()->user()->hasRole('super-admin') ? ['admin' => 'Admin'] : []) as $value => $label)<option value="{{ $value }}" @selected(($personHasOldInput ? old('role') : $primaryRole) === $value)>{{ __($label) }}</option>@endforeach</select><x-field-error name="role" :bag="$accessBag" :id="'role-error-'.$person->id" /></label>
