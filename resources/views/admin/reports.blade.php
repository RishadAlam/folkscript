    ...
    <h2 id="reports-heading" class="sr-only">{{ __('Reports') }}</h2>
        @forelse($reports as $report)
            ...
            <article class="moderation-item admin-report" id="report-{{ $report->id }}">
                ...
                @if($report->status === 'open')<div class="admin-case-actions">
                    <form method="post" action="{{ route('admin.reports.update', array_merge([$report], $returnQuery)) }}" class="moderation-actions">@csrf @method('PATCH')<button class="btn btn-outline" name="action" value="dismiss">{{ __('Dismiss report') }}</button><button class="btn btn-primary" name="action" value="resolve">{{ __('Mark reviewed') }}</button></form>
